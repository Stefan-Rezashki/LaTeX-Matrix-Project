...
def transpose_matrix(matrix):
    ...
    return matrix_to_latex(transposed_matrix);

#Ортогонална?
def is_orthogonal(matrix):
    sympy_matrix = sympy.Matrix(matrix)
    transpose_matrix = sympy_matrix.transpose()
    product_matrix = sympy_matrix * transpose_matrix

    identity_matrix = sympy.eye(sympy_matrix.shape[0])
    return product_matrix.simplify() == identity_matrix
